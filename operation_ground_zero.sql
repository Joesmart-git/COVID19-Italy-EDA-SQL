...
WHERE
  EXTRACT(YEAR FROM R.date) = 2022 
  -- Dynamic filtering based on Phase 1 peak results
  AND EXTRACT(MONTH from R.date) IN (1,3,7) 
  -- Defensive coding: prevents division-by-zero fatal errors
...
WITH weekly_record AS (
  -- Step 1: Establish clean weekly baselines to avoid daily reporting noise
  SELECT
...
SELECT
  ...
  prov_name,
  weekly_total_cases,
  -- Step 2: Dynamically subtract the previous week's cumulative total to derive net-new cases
...
WITH new_table AS (
  -- Merging all three geographical grains using composite joins
  SELECT
    DATE(P.date) AS day,
...
WHERE
  -- Dynamically fetches the peak infection day so the query remains future-proof
  NT.day = (
